    ...
    fn handle(&mut self, msg: RefreshTick, ctx: &mut Self::Context) -> Self::Result {
        ...
        let update_machine = state_action.then(move |_r, actor, ctx| {
            if let Some(interval) = actor.refresh_delay(prev_state) {
                ...
                let handle = Self::tick_later(ctx, pause);
                actor.tick_later_handle = Some(handle);
            } else {
                let update_timestamp = chrono::Utc::now();
                actor.state_changed = update_timestamp;
                // Cancel scheduled ticks, if any.
                actor.cancel_scheduled_ticks(&msg, ctx);
                Self::tick_now(ctx);
            }
            actix::fut::ready(())
        });
...
impl UpdateAgent {
    /// Cancel any ticks that have been scheduled e.g. by `tick_later()` by calling
    /// `cancel_future` on the handle in agent's `tick_later_handle` field.
    fn cancel_scheduled_ticks(&mut self, msg: &RefreshTick, ctx: &mut Context<Self>) {
        if let Some(handle) = self.tick_later_handle {
            match msg.command {
                // NB: During regular self refresh ticks, `tick_later_handle` is a handle to
                // a scheduled msg that has already been sent, so no need to cancel it.
                RefreshTickCommand::SelfTick => (),
                _ => {
